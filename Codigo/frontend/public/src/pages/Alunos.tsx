...
import { getAlunos, deleteAluno, updateAluno } from "../services/AlunosService";
import MonetizationOnIcon from '@mui/icons-material/MonetizationOn';
import AtualizaMoedas from "../components/AtualizaMoedas";
import AtualizarAluno from "./AtualizarAluno";
...
export default function Alunos() {
    ...
    const [idMoedasAluno, setIdMoedasAluno] = useState<number>();
    const [alunoMoedas, setAlunoMoedas] = useState<AlunoPayload>();
    const [OpenAtualizaMoedas, setOpenAtualizaMoedas] = useState<boolean>(false);
    ...
    useEffect(() => {
        ...
        // buscaData();
        setAlunos(aaa)
    }, [openAlunoDeletar, OpenAtualizaMoedas]);
    ...
    async function handleOpenMoedas() {
        setOpenAtualizaMoedas(true);
    }

    async function atualizaMoedas(valor: number) {
        console.log(valor)
        setOpenAtualizaMoedas(false)
        if (alunoMoedas && idMoedasAluno) {
            const alunoAtualizado = alunoMoedas
            alunoAtualizado.saldoMoedas += valor
            // try {
            //     await updateAluno(idMoedasAluno, alunoAtualizado)
            // }
            // catch (error) {
    ...
    return (
        <>
                                    ...
                                    {alunos.map((alunos) => (
                                        <StyledTableRow key={alunos.id}>
                                            ...
                                            <StyledTableCell align="center">
                                                ...
                                                <Button className="m-2" style={{ backgroundColor: "#ccaa32", border: "none" }} onClick={() => handleOpenMoedas()}><MonetizationOnIcon /></Button>
                                            </StyledTableCell>
                                        </StyledTableRow>
                                    ))}
                                </TableBody>
            ...
            <AtualizaMoedas
                id={idMoedasAluno}
                open={OpenAtualizaMoedas}
                onClose={() => setOpenAlunoDeletar(false)}
                onSubmitValue={(valor: number) => { atualizaMoedas(valor) }}
            />
        </>
    );
}
